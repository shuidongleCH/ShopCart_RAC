...
@implementation CDNumberCountView
...
- (void)setUI{
    ...
    _subButton.tag = 0;
    [[self.subButton rac_signalForControlEvents:UIControlEventTouchUpInside] subscribeNext:^(UIButton* x) {
        STRONG
    ...
    self.numberTT.layer.borderColor = kTextColor_92.CGColor;
    ...
    /************************** 内容改变 ****************************/
//    [[[NSNotificationCenter defaultCenter] rac_addObserverForName:UITextViewTextDidChangeNotification object:self.numberTT] subscribeNext:^(NSNotification* x) {
//        STRONG
...
//    }];
    ...
    /* 捆绑加减的enable */
    RAC(self.subButton,enabled) = [RACObserve(self, currentCountNumber) map:^id(NSNumber* subValue) {
        return @(subValue.integerValue > 1);
    }];
    RAC(self.addButton,enabled) = [RACObserve(self, currentCountNumber) map:^id(NSNumber* addValue) {
        return @(addValue.integerValue < self.totalNum);
    }];
    
    /* 内容颜色显示 */
    RAC(self.numberTT , textColor) = [RACObserve(self, totalNum) map:^id(NSNumber* totalValue) {
        return totalValue.integerValue == 0 ? [UIColor redColor] : [UIColor blackColor];
    }];
    /* 输入框文本展示 */
    RAC(self.numberTT,text) = [RACObserve(self, currentCountNumber) map:^id(NSNumber* value) {
